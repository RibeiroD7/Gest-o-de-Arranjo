...
O catálogo foi extraído do PDF oficial (sjjls-T) uma única vez e virou código;
...
class TestCatalogo:
    def test_tem_os_151_canticos_sem_buracos(self):
        assert TOTAL_CANTICOS == 151
        assert sorted(CANTICOS) == list(range(1, 152))
...
class TestRotulo:
    ...
    @pytest.mark.parametrize("invalido", [None, "", "abc", 0, 152, "12,5"])
